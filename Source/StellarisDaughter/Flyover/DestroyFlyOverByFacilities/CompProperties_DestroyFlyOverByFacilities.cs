...
namespace StellarisDaughter
{
    ...
    public class CompAbilityEffect_DestroyFlyOverByFacilities : CompAbilityEffect
    {
        ...
        private void DestroyFlyOversWithFacilities()
        {
            ...
            {
                Messages.Message("SD_DestroyFlyOver".Translate(flyOversWithFacilities.Count), parent.pawn, MessageTypeDefOf.PositiveEvent);
            }
            else
            {
                Messages.Message("SD_NoFlyOverWithFacilities".Translate(), parent.pawn, MessageTypeDefOf.NeutralEvent);
            }
        }
        ...
        public override bool Valid(LocalTargetInfo target, bool throwMessages = false)
        {
            ...
            if (flyOversWithFacilities.Count == 0)
            {
                if (throwMessages)
                {
                    Messages.Message("SD_NoFlyOverWithFacilities".Translate(), parent.pawn, MessageTypeDefOf.RejectInput);
                }
                return false;
            }
        ...
        public override string ExtraLabelMouseAttachment(LocalTargetInfo target)
        {
            ...
            {
                Log.Message($"[DestroyFlyOverByFacilities] Error in ExtraLabelMouseAttachment: {ex}");
