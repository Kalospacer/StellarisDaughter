...
namespace StellarisDaughter
{
    public class CompAbilityEffect_BlockedByFlyOverFacility : CompAbilityEffect
    {
        ...
        public override string ExtraLabelMouseAttachment(LocalTargetInfo target)
        {
            ...
            {
                Log.Message($"[BlockedByFlyOverFacility] Error in ExtraLabelMouseAttachment: {ex}");
                return "航道状态检查错误";
            }
        }
        ...
        private List<FlyOver> GetFlyOversWithFacilities()
        {
            ...
            {
                Log.Message($"[BlockedByFlyOverFacility] Error in GetFlyOversWithFacilities: {ex}");
        ...
        public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
        {
            ...
            if (HasFlyOverWithFacilities())
            {
                Log.Message($"[BlockedByFlyOverFacility] Attempted to use ability while FlyOvers are present");
                Messages.Message(Props.blockedMessage, parent.pawn, MessageTypeDefOf.RejectInput);
                return;
            }
