...
namespace StellarisDaughter
{
    public class CompAbilityEffect_CircularBombardment : CompAbilityEffect
    {
        ...
        public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
        {
            ...
            {
                Log.Message($"[CircularBombardment] Error starting bombardment: {ex}");
            }
        }
        ...
        private void LaunchSkyfaller(IntVec3 targetCell)
        {
            try
            {
                ...
                {
                    Log.Message($"[CircularBombardment] No skyfaller or projectile defined");
                }
            }
            catch (System.Exception ex)
            {
                Log.Message($"[CircularBombardment] Error launching at {targetCell}: {ex}");
            }
        }
