...
namespace StellarisDaughter
{
    public class CompAbilityEffect_Bombardment : CompAbilityEffect_WithDest
    {
        ...
        public override void Apply(LocalTargetInfo target, LocalTargetInfo dest)
        {
            ...
            {
                Log.Message($"[Bombardment] Error starting bombardment: {ex}");
            }
        }
        ...
        private void LaunchBombardment(IntVec3 targetCell)
        {
            try
            {
                ...
                {
                    Log.Message($"[Bombardment] No skyfaller or projectile defined for bombardment");
                }
            }
            catch (System.Exception ex)
            {
                Log.Message($"[Bombardment] Error launching bombardment at {targetCell}: {ex}");
            }
        }
